...
app.post("/tts", async (req, res) => {
  ...
  let geminiAImessages;
  if (!userMessage) {
    return res.status(400).json({ error: "Invalid request body" });
  }

  const geminiResponse = await runChat(userMessage, isFashionChat);
  const plainTextWithEmojis = geminiResponse
    .replace(/<[^>]*>/g, "") // Remove HTML tags
    .replace(/\*/g, "");
  console.log(userMessage);
  console.log(plainTextWithEmojis);

  try {
    const response = await axios.post("http://localhost:8000/detect-emotion/", {
      text: plainTextWithEmojis,
    });
    const facialExpression = response.data.facialExpression;
    const animation = response.data.animation;
    console.log(facialExpression, animation);
    geminiAImessages = [
      {
        text: plainTextWithEmojis,
    ...
    console.error("Error occured in the api", error);
  }
  try {
    geminiAImessages = await lipSync({ messages: geminiAImessages });
    res.send({ messages: geminiAImessages });
  } catch (error) {
